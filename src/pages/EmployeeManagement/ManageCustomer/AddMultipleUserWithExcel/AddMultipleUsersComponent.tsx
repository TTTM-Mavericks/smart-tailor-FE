...
const AddMultipleComponentWithExcel: React.FC<AddUserWithMultipleExcelFormProps> = ({ closeMultipleCard, addNewUser }) => {
                    ...
                    if (event.target && event.target.result) {
                        ...
                        const uniqueData = jsonData.filter((item, index, self) => {
                            const isDuplicate = self.findIndex((t) => (
                                t.name === item.name && t.phone === item.phone && t.email === item.email && t.address === item.address
                            )) !== index;
                        ...
                        const updatedData = jsonData.map(item => {
                            const isDuplicate = duplicateData.some(d => d.name === item.name && d.phone === item.phone && d.email === item.email && d.address === item.address);
    ...
    const _handleDownloadFullData = () => {
        ...
        const uniqueCols = ['name', 'email', 'phone', 'address']; // Cột không được trùng lặp
                                    ...
                                    <tr key={index}>
                                        ...
                                        <td style={{ border: '1px solid #ddd', padding: '8px', color: data.error ? 'red' : 'green' }}>
                                            {(() => {
                                                ...
                                                const isAddressDuplicate = excelData.some((item, i) => i !== index && item.address === data.address);
                                                const hasNullValues = Object.values(data).some(value => value === null || value === '');

                                                if (isNameDuplicate || isPhoneDuplicate || isEmailDuplicate || isAddressDuplicate || hasNullValues) {
                                                    const errorMessage = [];
                                                    ...
                                                    if (isAddressDuplicate) errorMessage.push('Duplicate Address');
