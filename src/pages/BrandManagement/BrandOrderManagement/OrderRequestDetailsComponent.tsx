...
const OrderRequestDetailsComponent: React.FC<OrderDetailsProps> = ({ order, design, designDetail, brand }) => {
    ...
    const [remainingTime, setRemainingTime] = useState(5000 * 1000);
    ...
    useEffect(() => {
        const designDetailRelated = designDetail.filter((item) => item.detailStatus === false)

        const extractedSizes = designDetailRelated.map((item: any) => {
