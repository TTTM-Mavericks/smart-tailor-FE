...
import style from './AccountantManageFinedPaymentComponentStyle.module.scss'
...
const AccountantManageFinedPaymentComponent: React.FC = () => {
    // ---------------UseState---------------//
    ...
    const __handleFetchOrderData = async (userID: any) => {
        setIsLoading(true)
        try {
            const response = await api.get(`${versionEndpoints.v1 + featuresEndpoints.payment + functionEndpoints.payment.getAllPayment}`, null, __getToken());
            if (response.status === 200) {
                console.log(response.data);
    ...
    */
    ...
    const applyFilters = (transaction: PaymentOrderInterface) => {
        console.log('transaction.orderID.includes(filters.paymentType): ', transaction.orderID.includes(filters.paymentType));
        return (
            ...
            (filters.paymentType === '' || transaction.paymentType.includes(filters.paymentType))
                ...
                <div style={{ width: '100%' }} className="max-w-6xl mx-auto p-4 md:p-6 min-h-screen">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                        ...
                        {selectedFilters.includes('Payment Status') && (
                            <div className="filter-item">
                                <label htmlFor="paymentstatus" className="block mb-2 text-sm font-medium text-gray-700">Order Status</label>
                                <select
                                    id="paymentstatus"
                                    name="status"
                        ...
                        {selectedFilters.includes('Payment Types') && (
                            <div className="filter-item">
                                <label htmlFor="paymentType" className="block mb-2 text-sm font-medium text-gray-700">Order Status</label>
                                <select
                                    id="paymentType"
                                    name="paymentType"
                                ...
                                >
                                    ...
                                    <option value={'BRAND_INVOICE'}>BRAND_INVOICE</option>
                    ...
                    {fulldataTransactionResposne?.filter(applyFilters).map((transaction) => (
                        ...
                        <div className="bg-white rounded-xl shadow-md p-4 md:p-6 mb-4 md:mb-8 transform transition-all hover:shadow-lg">
                            ...
                            {transaction.paymentType === 'BRAND_INVOICE' && transaction.paymentStatus === false && (
                                <>
                                    <button
                                        type="submit"
...
export default AccountantManageFinedPaymentComponent;
