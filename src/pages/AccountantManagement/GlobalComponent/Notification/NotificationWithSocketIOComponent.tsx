...
const NotificationWithSocketIOScreen = () => {
    ...
    return (
        <div>
            <IconButton onClick={_handleClickNotification}>
                <Badge
            ...
            >
                {data.map((item) => (
                    <div key={item.notiID} style={{ borderRadius: '4px', marginBottom: '8px', position: 'relative', backgroundColor: item.status ? 'rgba(162,222,82,0.1)' : 'rgba(234,49,62,0.1)' }} onClick={() => _handleMarkIsRead(item)} >
