...
import { OrderInterface, PaymentInterface } from '../../../models/OrderModel';
...
const exampleOrders: OrderInterface[] = [
    ...
    },
    {
        ...
        design: {
            designID: '1a2b3c4d-5e6f-7g8h-9i0j-1k2l3m4n5o6p',
            imgUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS4PHI5muWT_qjOt9JhBhYZsxfHd_5dJ0xz7zW58UfOCkg-SN1opuEk1HWEpbCxp2_--LM&usqp=CAU'
        },
...
const OrderHistory: React.FC = () => {
    ...
    const [currentPaymentData, setCurrentPaymentData] = useState<PaymentInterface>();
    ...
    const __handleExtendTranscation = (orderID: string) => {
     ...
     * @param payment 
     */
    const __handleViewInvoiceClick = (payment: PaymentInterface) => {
     ...
     * @returns 
     */
    const renderStatusIcon = (item: PaymentInterface) => {
        if (item.paymentStatus === true) {
            return (
                <div className="flex items-center text-green-600" style={{ backgroundColor: `rgba($color: ${greenColor}, $alpha: 0.7)` }}>
                    ...
                    <span className="font-medium">Đã thanh toán: {item.lastModifiedDate}</span>
                </div>
            );
        }
                            ...
                            {isExtendTransaction[order.orderID] && order?.payment?.map((payment, itemIndex) => (
                                <div key={itemIndex} className="flex flex-col md:flex-row items-start md:items-center mb-4 md:mb-6 border-b pb-4 md:pb-6">
                                    <div className="flex-shrink-0">
                                        <img className="w-32 h-28 md:w-35 md:h-40 rounded-lg shadow-md" src={order?.design?.imgUrl} alt={`Image `} />
                                    </div>
