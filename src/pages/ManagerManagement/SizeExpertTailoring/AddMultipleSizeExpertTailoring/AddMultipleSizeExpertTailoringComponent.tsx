...
const AddMultipleSizeExpertTailoringComponentWithExcel: React.FC<AddSizeExpertTailoringWithMultipleExcelFormProps> = ({ closeMultipleCard }) => {
    ...
    */
                ...
                try {
                    if (event.target && event.target.result) {
                        ...
                        updatedData.forEach(item => {
                            console.log(`Category: ${item.Expert_Tailoring_Name}, Material Name: ${item.Size_Name}`);
                        });
    ...
    const _handleConfirm = async () => {
        ...
        const duplicates = checkForDuplicates(excelData, ['Expert_Tailoring_Name', 'Size_Name']);

        // Check for invalid entries
        const invalidEntries = excelData.some(item =>
            !item.Expert_Tailoring_Name ||
            !item.Size_Name ||
            !isValidExpertTailoringName(item.Size_Name) ||
    ...
    const _handleDownloadErrorData = async () => {
        ...
        const worksheet = workBook.addWorksheet('Size Expert Tailoring');

        worksheet.columns = [
            { header: 'Expert_Tailoring_Name', key: 'Expert_Tailoring_Name', width: 20 },
            { header: 'Size_Name', key: 'Size_Name', width: 20 },
            { header: 'Min_Fabric', key: 'Min_Fabric', width: 20 },
            { header: 'Max_Fabric', key: 'Max_Fabric', width: 20 },
            { header: 'Unit', key: 'Unit', width: 20 }
        ];

        // Insert a custom header row above the defined columns
        worksheet.insertRow(1, ['Size EXPERT TAILORING']);
        ...
        const rows = excelData.map(item => ({
            ...
            Min_Fabric: item.Min_Fabric,
            Max_Fabric: item.Max_Fabric,
            Unit: item.Unit,
            error: item.error
        }));
        ...
        const rowsWithError = rows.filter(row => {
            ...
            const isSizeNamel = row.Size_Name === null || row.Size_Name === undefined || typeof row.Size_Name === 'number';
            return hasNullValues || isExpertTailoringName || isSizeNamel;
        });
        ...
        const duplicates = checkForDuplicates(sortedRows, ['Expert_Tailoring_Name', 'Size_Name']);
        ...
        sortedRows.forEach((row, rowIndex) => {
            ...
            excelRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
                ...
                if (columnName === 'Expert_Tailoring_Name' || columnName === 'Size_Name') {
                    if (duplicates.has(`${row.Expert_Tailoring_Name}|${row.Size_Name}`)) {
                        const duplicateRowNumber = sortedRows.findIndex(r => r.Expert_Tailoring_Name === row.Expert_Tailoring_Name && r.Size_Name === row.Size_Name && r !== row) + 2;
                        cell.fill = {
                            type: 'pattern',
                            pattern: 'solid',
                ...
                } else if (columnName === 'Expert_Tailoring_Name' || columnName === 'Size_Name' || columnName === 'Unit') {
                    if (!value) {
                        cell.fill = {
                            type: 'pattern',
                            pattern: 'solid',
                        ...
                        cell.value = `${value} (Invalid HS Code)`;
                    } else if (columnName === 'Size_Name' && typeof value === 'string') {
                        cell.fill = {
                            type: 'pattern',
                            pattern: 'solid',
                            fgColor: { argb: 'FFFF00' }, // Red fill for invalid Size_Name
                        };
                        cell.value = `${value} (Invalid Base Price)`;
                    } else if (columnName === 'Size_Name' && parseFloat(value) <= 0) {
                        cell.fill = {
                            type: 'pattern',
                            pattern: 'solid',
                            fgColor: { argb: 'FFFF00' }, // Red fill for invalid Expert_Tailoring_Name
        ...
        a.href = url;
        a.download = "Size_Exper_Tailoring_Edit.xlsx";
        a.click();
    ...
    const _handleDownloadSampleExcelFile = () => {
        const url = `${baseURL + versionEndpoints.v1 + featuresEndpoints.sizeExpertTailoring + functionEndpoints.sizeExpertTailoring.generateSizeExpertTailoringSample}`; // Replace with your API endpoint
        ...
        axios({
            ...
            .then(response => {
                ...
                a.href = url;
                a.download = 'Size_Expert_Tailoring_Sample_File.xlsx'; // Replace with your desired file name
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ backgroundColor: colors.primary[100] }}>
                                    ...
                                    <th style={{ border: '1px solid #ddd', padding: '8px' }}>Size Name</th>
                            ...
                            <tbody>
                                {excelData.map((data, index) => (
                                    <tr key={index}>
                                        <td style={{ border: '1px solid #ddd', padding: '8px', color: data.Expert_Tailoring_Name ? colors.primary[200] : 'red' }} >{data.Expert_Tailoring_Name || 'Null'}</td>
                                        <td style={{ border: '1px solid #ddd', padding: '8px', color: data.Size_Name ? colors.primary[200] : 'red' }} >{data.Size_Name || 'Null'}</td>
...
export default AddMultipleSizeExpertTailoringComponentWithExcel;
